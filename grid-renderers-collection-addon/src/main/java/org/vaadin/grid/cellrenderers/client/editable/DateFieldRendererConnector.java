...
public class DateFieldRendererConnector extends ClickableRendererConnector<Date> {
    ...
    public class DateFieldClientRenderer extends ClickableRenderer<Date, VMyPopupCalendar> {
        ...
        @Override
        public void render(final RendererCellReference cell, final Date selectedValue, final VMyPopupCalendar dateField) {
            final Element e = dateField.getElement();

            getState().value = selectedValue;

            if (e.getPropertyString(EditableRendererClientUtil.ROW_KEY_PROPERTY) != getRowKey((JsonObject) cell.getRow())) {
            ...
            if (e.getPropertyString(EditableRendererClientUtil.COLUMN_ID_PROPERTY) != getColumnId(EditableRendererClientUtil.getGridFromParent(getParent())
                .getColumn(cell.getColumnIndex()))) {
                e.setPropertyString(EditableRendererClientUtil.COLUMN_ID_PROPERTY, getColumnId(EditableRendererClientUtil.getGridFromParent(getParent())
                    .getColumn(cell.getColumnIndex())));
            }
            ...
            DateFieldRendererConnector.this.rpc.onRender(new CellId(e.getPropertyString(EditableRendererClientUtil.ROW_KEY_PROPERTY),
        ...
        public VMyPopupCalendar createWidget() {
            ...
            dateField.addDomHandler(new ChangeHandler() {
                ...
                public void onChange(final ChangeEvent changeEvent) {
                    final Element e = dateField.getElement();
                    DateFieldRendererConnector.this.rpc.onChange(e.getPropertyString(EditableRendererClientUtil.ROW_KEY_PROPERTY),
                                                                 e.getPropertyString(EditableRendererClientUtil.COLUMN_ID_PROPERTY), dateField.getDate());
                }
            }, ChangeEvent.getType());
